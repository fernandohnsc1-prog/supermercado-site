...
export default function SiteHeader() {
  ...
  useEffect(() => {
    fetch('/api/auth/check')
      .then((r) => r.json())
      .then((data) => { if (data.autenticado) setIsAdmin(true) })
      .catch(() => {})
  }, [])
          ...
          <nav className="hidden lg:flex items-center gap-5">
            ...
            {isAdmin && (
              <Link
                ...
                className="flex items-center gap-2 bg-orange-600 hover:bg-orange-700 text-white font-bold text-sm px-5 py-2.5 rounded-full transition-all duration-200 hover:scale-105 shadow-md"
              >
                <span>⚙️</span>
                Painel Admin
              </Link>
          ...
          <nav className="lg:hidden pb-4 border-t border-gray-100 pt-3">
            ...
            {isAdmin && (
              <Link
                href="/admin/dashboard"
                onClick={() => setMenuAberto(false)}
                className="flex items-center gap-2 bg-orange-600 text-white font-bold text-sm px-5 py-2.5 rounded-full mt-3 w-fit"
              >
                <span>⚙️</span>
                Painel Admin
              </Link>
