...
export default function EncartesSection() {
  ...
  const televendasMsg = encodeURIComponent('Olá! Vim pelo encarte do site e gostaria de mais informações.')
  const televendasLink = `https://wa.me/${TELEVENDAS}?text=${televendasMsg}`
  ...
  return (
    <section id="encartes" className="py-16 bg-white">
      ...
      {/* Modal viewer — zoom, scroll, fullscreen, share */}
      {encarteAberto && (
        <div
          className={`fixed inset-0 z-50 flex flex-col ${fullscreen ? 'bg-black' : 'bg-black/90'}`}
          onClick={(e) => { if (e.target === e.currentTarget) fecharEncarte() }}
        >
          {/* Top bar */}
          <div className={`flex items-center justify-between px-4 py-3 ${fullscreen ? 'bg-black/50' : ''} text-white`}>
            ...
            <div className="flex items-center gap-2">
              {/* Zoom controls */}
              <button onClick={() => handleZoom(-0.25)} className="w-8 h-8 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 transition text-sm font-bold" title="Diminuir zoom">−</button>
              <span className="text-xs text-gray-300 min-w-[3rem] text-center">{Math.round(zoom * 100)}%</span>
              <button onClick={() => handleZoom(0.25)} className="w-8 h-8 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 transition text-sm font-bold" title="Aumentar zoom">+</button>
              <div className="w-px h-6 bg-white/20 mx-1" />
              {/* Fullscreen */}
              <button onClick={toggleFullscreen} className="w-8 h-8 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 transition" title={fullscreen ? 'Sair da tela cheia' : 'Tela cheia'}>
                {fullscreen ? (
              ...
              {/* Close */}
              <button onClick={fecharEncarte} className="w-8 h-8 flex items-center justify-center rounded-full bg-red-600/80 hover:bg-red-600 transition text-sm font-bold" title="Fechar">✕</button>
            </div>
          </div>

          {/* Image viewer with scroll and zoom */}
          <div className="flex-1 overflow-auto flex items-start justify-center p-4">
            <img
              src={encarteAberto.imagens[imagemIdx]?.imagem_url}
              alt={encarteAberto.titulo}
          ...
          {encarteAberto.imagens.length > 1 && (
            <div className="flex items-center justify-center gap-2 py-3 bg-black/50">
              <button
                onClick={() => setImagemIdx(Math.max(0, imagemIdx - 1))}
              ...
              <button
                onClick={() => setImagemIdx(Math.min(encarteAberto.imagens.length - 1, imagemIdx + 1))}
